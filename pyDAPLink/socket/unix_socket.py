...
class UnixServer(Server):
    ...
    def init(self):
        ...
        # Create a pipe for signaling that an accept should be canceled
        self._shutdown_pipe = os.pipe()
        self._isalive = True

    def accept(self):
        ready, _, _ = select([self._socket, self._shutdown_pipe[0]], [], [])
        if self._shutdown_pipe[0] in ready:
            return None
    ...
    def shutdown(self):
        os.write(self._shutdown_pipe[1], 'x')
        self._isalive = False
